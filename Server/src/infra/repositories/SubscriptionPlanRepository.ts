...
import { ISubscriptionPlanDocument, SubscriptionPlanModel } from '../database/SubscriptionPlan.model';
  ...
  implements ISubscriptionPlanRepository
{
  ...
  async findByTier(tier: string): Promise<SubscriptionPlan | null> {
    const doc = await SubscriptionPlanModel.findOne({ tier: tier.toUpperCase() });
    return doc ? subscriptionPlanMapper.toDomain(doc) : null;
  }

  async findAllActive(): Promise<SubscriptionPlan[]> {
    const docs = await SubscriptionPlanModel.find({ isActive: true }).sort({ 'price.monthly': 1 });
  ...
  async findByStripeId(stripeId: string): Promise<SubscriptionPlan | null> {
    const doc = await SubscriptionPlanModel.findOne({
      $or: [
        { 'stripeId.monthly': stripeId },
        { 'stripeId.yearly': stripeId },
      ],
    });
    return doc ? subscriptionPlanMapper.toDomain(doc) : null;
  }
