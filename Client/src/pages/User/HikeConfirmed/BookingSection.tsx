  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-100/50 p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-slate-900 tracking-tight">{rider.name}</h3>
              <div className="flex items-center gap-2 mt-1">
                <div className="flex items-center gap-1 bg-slate-900 px-2 py-0.5 rounded-md">
                  <span className="text-yellow-400 text-xs">★</span>
                  <span className="text-xs font-bold text-white">{rider.rating}</span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-md border border-slate-100 p-4 flex items-center justify-between hover:shadow-lg transition-all duration-300 group">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-slate-50 rounded-xl text-slate-700 group-hover:bg-slate-100 transition-colors">
                {['bike', 'scooter', 'motorcycle'].some(type => rider.vehicleModel.toLowerCase().includes(type)) ? (
              ...
              <div>
                <p className="text-sm font-bold text-slate-400 uppercase tracking-wider">Vehicle</p>
                <p className="text-lg font-bold text-slate-900">{rider.vehicleModel}</p>
              </div>
            </div>
            <div className="px-4 py-2 bg-slate-900 rounded-lg shadow-sm">
              <span className="font-mono font-bold text-white tracking-widest">{rider.vehicleNumber}</span>
            </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 pt-0 -mt-1.5">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-bold text-emerald-600 tracking-wider uppercase bg-emerald-50 px-2 py-0.5 rounded-md">PICKUP</span>
                </div>
                <p className="font-bold text-slate-900 text-lg leading-snug">
                  {rideDetails.pickupAddress}
                </p>
                <div className="flex items-center gap-3 mt-1">
                   <p className="text-sm font-medium text-slate-500">Departure: {departure}</p>
                   {rideOtp.otp && (
                    ...
                    {!rideOtp.otp && (
                      ...
                      >
                         {rideOtp.loading ? <Loader2 className="w-3 h-3 animate-spin"/> : 'View OTP'}
                      </button>
                    )}
                </div>
              </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 pt-0 -mt-1.5">
                ...
                <p className="text-sm font-medium text-slate-500 mt-1">Est. {estimation}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Trip Stats */}
        <div className="bg-white rounded-2xl shadow-lg border border-slate-100/50 p-6">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Trip Summary</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-xl border border-slate-100 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all duration-300 group cursor-default">
              ...
              <p className="text-xl font-bold text-slate-900 group-hover:text-indigo-900">{Math.round(rideDetails.duration)}<span className="text-sm font-medium text-slate-400 ml-1">min</span></p>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-100 hover:border-purple-100 hover:bg-purple-50/30 transition-all duration-300 group cursor-default">
              ...
              <p className="text-xl font-bold text-slate-900 group-hover:text-purple-900">{rideDetails.distance}<span className="text-sm font-medium text-slate-400 ml-1">km</span></p>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-100 hover:border-emerald-100 hover:bg-emerald-50/30 transition-all duration-300 group cursor-default">
              ...
              <p className="text-xl font-bold text-slate-900 group-hover:text-emerald-900">{rideBooking.seatsBooked}</p>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-100 hover:border-orange-100 hover:bg-orange-50/30 transition-all duration-300 group cursor-default">
              ...
              <p className="text-xl font-bold text-slate-900 group-hover:text-orange-900">₹{rideBooking.amount.toFixed(0)}</p>
            </div>
          </div>
        </div>
      </div>
